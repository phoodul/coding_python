...
# ==========================================
st.set_page_config(page_title="Neuro-Sim V10.0: Dosage Unit Fix", layout="wide")
...
st.title("🧠 Neuro-Sim V10.0: Dosage Unit Fix")
...
with st.expander("🛠️ Step 1. 환자 민감도 설정", expanded=False):
    ...
    with c2:
        ...
        for i, item in enumerate(st.session_state.history):
            ...
            # [Update] Number input step and format
...
for idx, d in enumerate(current_drugs):
    with cols[idx]:
        ...
        # [Update] Step Logic: High dose -> 1.0, Low dose -> 0.5
...
if has_aripiprazole:
    eff_a = 1.0 # Stabilized
    eff_b = 1.0
    st.success("🧪 **Stabilization Active:** 아리피프라졸 사용으로 인해 도파민 초민감성이 억제되어 **수용체 효율이 100%로 정상화**되었습니다.")
else:
    eff_a = 1.5
    eff_b = 2.0
    st.warning("⚠️ **Supersensitivity Risk:** 고전적인 항정신병제 장기 사용으로 인해 수용체 효율이 **150% ~ 200%**로 증가한 상태입니다.")
...
# Type i
st.subheader(f"Type B (효율 {eff_b*100:.0f}%)")
...
<div class="legend-box">
    <h4>✅ V10.0 업데이트</h4>
    <ul>
        <li><b>용량 조절 개선:</b> 경구약은 <b>0.5mg</b> 단위, 주사제는 <b>1.0mg</b> 단위로 조절되며 소수점 첫째 자리까지만 표시됩니다.</li>
    </ul>
</div>
""", unsafe_allow_html=True)