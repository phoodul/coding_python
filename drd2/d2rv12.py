...
# 0. UI 설정
# ==========================================
st.set_page_config(page_title="Neuro-Sim V12.0: Layout Fix", layout="wide")
...
st.title("🧠 Neuro-Sim V12.0: Layout & Terminology Fix")
...
# --- Step 2: Acute State ---
# [Fix 1] 명칭 변경: 만성 -> 급성
st.header("Step 2. 급성 조현병 (Acute Schizophrenia, $\epsilon=50\%$)")
...
if has_aripiprazole:
    eff_a = 1.1 # 110%
    eff_b = 1.2 # 120%
    st.info("🧪 **Stabilization Active:** 아리피프라졸 사용으로 효율이 110%~120% 수준으로 조정되었습니다.")
else:
    eff_a = 1.5
    eff_b = 2.0
    st.warning("⚠️ **High Risk:** 수용체 효율 150%~200% 상태입니다.")

# [Fix 2] 레이아웃 순서 강제: Header -> Columns -> Content
...
<div class="legend-box">
    <h4>✅ V12.0 수정 완료</h4>
    <ul>
        <li><b>명칭 변경:</b> Step 2가 "급성 조현병"으로 변경되었습니다.</li>
        <li><b>레이아웃 수정:</b> Step 3에서 Type A/i 헤더가 그래프보다 먼저 나오도록 순서를 바로잡았습니다.</li>
    </ul>
</div>
""", unsafe_allow_html=True)