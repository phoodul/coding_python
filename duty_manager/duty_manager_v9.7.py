...
class DutyAppV97(QMainWindow):
    ...
    def init_ui(self):
        ...
        h2 = QHBoxLayout(); self.cb_req = QCheckBox("🔴 Request 모드"); self.cb_req.toggled.connect(lambda v: setattr(self, 'request_mode', v)); self.btn_run = QPushButton("🚀 RUN"); self.btn_run.clicked.connect(self.run_algo); self.btn_pr2 = QPushButton("🖨️ 인쇄"); self.btn_pr2.clicked.connect(lambda: self.print_preview(self.table2)); self.btn_next2 = QPushButton("다음 달 ▶"); self.btn_next2.clicked.connect(self.go_next)
        ...
        self.table2 = QTableWidget(); self.table2.cellClicked.connect(self.on_table2_click); self.table2.itemChanged.connect(self.on_table2_changed); l2.addWidget(self.table2); self.tabs.addTab(p2,"테이블 2")
        # T3
        ...
        self.table3 = QTableWidget(); self.table3.setItemDelegate(VerticalTextDelegate()); l3.addWidget(self.table3); self.footer = QLabel(); self.footer.setFont(QFont("Malgun Gothic",9)); l3.addWidget(self.footer); self.tabs.addTab(p3,"테이블 3")
